<template name="components">
	<view>
		...
		<cu-custom bgColor="bg-white">
			<block slot="content">
				<view class="cu-bar  search bg-white" style="height: 100%;">
					<view class="action text-black margin-right" style="width:100upx;">
						<text class="text-cut">{{location}}</text>
					</view>
					<view class="search-form round" style="margin: 0 0rpx;">
						<text class="cuIcon-search"></text>
						<input @confirm="searchBykeywords" :adjust-position="false" type="text" placeholder="输入搜索关键词" confirm-type="search"></input>
					</view>
					<image @tap="swithTab" class="cu-avatar round margin-lr" :src="avatarUrl" mode="aspectFit"></image>
				</view>
			</block>
		</cu-custom>
		<view class="tui-screen-top solid-bottom">
			...
			<view class="tui-dropdownlist" :class="[selectH1>0?'tui-dropdownlist-show':'']">
				<view class="tui-dropdownlist-item tui-icon-middle" :class="[item.selected?'tui-bold':'']" v-for="(item,index) in dropdownList1"
				 :key="index" @tap="dropdownItem2" :data-name="item.name" :data-value="item.value" :data-index="index">
			...
			<view class="tui-dropdownlist" :class="[selectH2>0?'tui-dropdownlist-show':'']">
				<view class="tui-dropdownlist-item tui-icon-middle" :class="[item.selected?'tui-bold':'']" v-for="(item,index) in dropdownList2"
				 :key="index" @tap="dropdownItem4" :data-name="item.name" :data-value="item.value" :data-index="index">
					<view class="margin-tb-sm">
						<text class="text-theme">{{item.name}}</text>
					</view>
					<text class="cuIcon-check" v-if="item.selected"></text>
		...
		<!-- #endif -->
		<!-- #ifdef MP-WEIXIN -->
		<view class="cu-bar fixed search bg-white solid-top">
			<view class="action text-black margin-right" style="width:100upx;">
				<text class="text-cut">{{location}}</text>
			</view>
			<view class="search-form round" style="margin: 0 0rpx;">
			...
			<view @tap="swithTab" class="cu-avatar round margin-lr" :style="'background-image:url('+avatarUrl+')'"></view>
		</view>
		<view class="tui-header-screen margin-top">
			<view class="tui-screen-top solid-bottom">
				<view class="tui-top-item tui-icon-ml " :class="[tabIndex==1?'tui-active tui-bold':'']" data-index="1" @tap="screen">
					<view class="text-lg"><text class="text-df">{{selectedName[0]}}</text><text :class="selectH1>0?'cuIcon-triangleupfill':'cuIcon-triangledownfill'"></text></view>
				</view>
		...
		onLoad(e) {
			...
			if (this.lat != 0) {
				this.$api.get(
					...
					this.lat + ',' + this.long,
					function callbacks(res) {
						that.location = res.result.addressComponent.city;
					})
			}
		},
		methods: {
