<template name="mine">
	<view style="background-color: #f2f2f2;">
		<view class="bg-white">
			<view class="bg-theme padding-top padding-left" style="height: 300upx;border-bottom-right-radius: 80upx;border-bottom-left-radius: 80upx;">
				<button @tap="logout" class="cu-btn text-white bg-transparent" style="position: absolute;top: 20upx;right:40upx">
					退出登陆
				</button>
				<view class="flex margin-left  align-center">
					...
					<view class="white margin-left-sm">
						<view class="text-lg">{{nickname}}</view>
					</view>
				</view>
			</view>
			<view style="position: absolute;top:200upx;width:90%;left:37upx;" class="bg-white radius shadow">
				<view class="cu-list grid no-border col-5">
					<view @tap="navigateTo(item.url)" class="cu-item" v-for="(item,index) in info" :key="index">
						<text :class="index==0?'text-price':''">{{item.number}}</text>
						<text>{{item.name}}</text>
					</view>
				</view>
			</view>
		</view>
		<view class="bg-white" style="padding-top: 120upx;">
			<text class="text-bold text-black margin-left" v-if="changygn !==undefined && changygn.length > 0 ">常用功能</text>
		...
		<!-- <view class="solids-top"></view> -->
		<view class="bg-white margin-top-sm padding-top">
			<text class="text-bold text-black margin-left" v-if="otherFuns !==undefined && otherFuns.length > 0 ">拓展功能</text>
			<view class="cu-list grid no-border col-3">
				<view @tap="navigateTo(item.url)" class="cu-item" v-for="(item,index) in otherFuns" :key="index">
					<view :class="['cuIcon-' + item.cuIcon,'text-' + item.color]">
					</view>
					<text>{{item.name}}</text>
				</view>
	...
	export default {
		...
		onLoad() {
			...
			jweixin.config({
			  ...
			  jsApiList: ['checkJsApi','translateVoice','scanQRCode'] // 必填，需要使用的JS接口列表
			});
			// #endif
		},
		onTabItemTap(e) {
