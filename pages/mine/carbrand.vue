<template>
	<view class="tui-page">
		...
		<scroll-view class="tui-scrollList" scroll-y :scroll-into-view="scrollViewId" :style="{height:winHeight + 'px'}"
		 @scroll="onScroll">
			...
			<view class="tui-searchbox">
				<view class="tui-search-input tui-skeleton-rect" @tap="search">
					<icon type="search" :size='15' color='#999'></icon>
					<text class="tui-search-text">搜索</text>
				</view>
			</view>
			<view class="cu-list grid col-5 no-border" >
				<view @tap="chooceThis" class="cu-item " v-for="(item,index) in 12" :key="index">
					<view class="">
						<image src="../../static/bm.png" mode="aspectFit" class="tui-img tui-skeleton-circular"></image>
					</view>
					<view class="tui-name tui-skeleton-rect">宝马</view>
				</view>
			</view>
			<!--searchbox-->
			<!--联系人列表-->
				...
				<block v-for="(list,index) in lists" :key="index">
					<tui-sticky :scrollTop="scrollTop" stickyHeight="66rpx" v-if="list.data[0]">
						...
						<template v-slot:content>
							<block @tap="chooceThis" v-for="(item,index2) in list.data" :key="index2">
								<tui-cell :last="last(list.data,index2)" @click="detail">
									<image mode="aspectFit" :src="'../../static/'+((index2%2===0 && index2!==0)?'bm.png':'ad.png')"
									 class="tui-img "></image>
									<view class="tui-name margin-left">{{item.name}}</view>
								</tui-cell>
							</block>
						</template>
					</tui-sticky>
			...
			<!--联系人列表-->
			<view class="tui-footer tui-skeleton-rect">120种品牌</view>
			<view class="tui-safearea-bottom"></view>
		</scroll-view>
		<view class="tui-indexed-list-bar tui-skeleton-rect" :style="{height:indexBarHeight+'px'}" @touchstart.stop="touchStart"
		...
		<view class="tui-indexed-list-alert tui-skeleton-rect" v-if="touchmove && lists[touchmoveIndex].letter">
